...
**핵심 포인트 — 결제 정합성 확보**
- 금액 위변조 방지: 요청 금액, DB 저장 금액, PG 승인 금액을 PG 호출 전/후 2단계로 체인 검증
...
- **문제 인식** — 기존 흐름은 토스 `confirm()`으로 결제를 **승인한 뒤** 금액·상태를 검증하는 구조인데, 위변조된 금액이나 중복 요청도 일단 PG 승인이 나간 뒤에야 걸러져서 잘못된 건은 별도의 결제 취소(보상 트랜잭션)가 필요해 흐름이 복잡해지고 정합성 리스크 존재
- **개선 — 검증을 두 계층으로 분리**
